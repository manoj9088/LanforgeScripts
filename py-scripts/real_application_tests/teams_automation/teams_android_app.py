...
class TeamsAndroidApp:
    ...
    def enable_video(self):
        ...
        while (
            ...
            if count > 60:
                self.logger.error(
                    f"Unable to find Video Turn on Button or Video Turn of Button{self.participant_name} ({self.serial})"
                )
                return
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.device:
            ...
            logging.error(
                "No device serial specified. Please provide a device serial using the --device argument."
            )
            sys.exit(1)
